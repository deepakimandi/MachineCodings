package org.example.SnakeAndLadders.service;

import lombok.Getter;
import org.example.SnakeAndLadders.enums.ObstacleType;
import org.example.SnakeAndLadders.factory.ObstacleFactory;
import org.example.SnakeAndLadders.model.*;
...
public class Game {
    ...
    private void generateObstacles(int count, ObstacleType type) {
        ...
        while (count > 0) {
            int up = random.nextInt(size - 1) + 2;      // Ensures up >= 2
            int down = random.nextInt(up - 1) + 1;      // Ensures down < up
    ...
    public void startGame() {
        ...
        while (players.size() > 1) {
            ...
            if (newPosition == board.getSize()) {
                ...
                // Player is removed from the queue since they won
            } else {
                players.offer(currPlayer);
            }
